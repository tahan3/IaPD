	...
	if (message == WM_DEVICECHANGE)
	{
		if (wParam == DBT_DEVICEARRIVAL)
		{
			...
			if (lpdb->dbch_devicetype == DBT_DEVTYP_DEVICEINTERFACE)
			{
				...
				cout << "Был подключен девайс " << getFriendlyName(info) << endl;
		...
		if (wParam == DBT_DEVICEQUERYREMOVEFAILED)
		{
			...
			{
				PDEV_BROADCAST_HANDLE info = (PDEV_BROADCAST_HANDLE)lpdb;
				cout << "Невозможно безопасно извлечь \"" << handleToFriendlyName(info->dbch_handle) << "\"\n";
		...
		if (wParam == DBT_DEVICEREMOVECOMPLETE)
		{
			...
			{
				PDEV_BROADCAST_DEVICEINTERFACE_A info = (PDEV_BROADCAST_DEVICEINTERFACE_A)lpdb;
				cout << "Девайс \"" << getFriendlyName(info) << "\" был извлечен"
					<< (nameToSafety((wchar_t*)info->dbcc_name) ? " безопасно!" : "!") << '\n';
...
DWORD WINAPI threadProcess(LPVOID lpParameter)
{
	...
	// регистрируем приложение в сиситеме, чтобы оно получало сообщения об устройвах
...
int main()
{
	...
	CreateThread(NULL, NULL, threadProcess, NULL, NULL, NULL);
	Sleep(1000);
	...
	while (true)
	{
		...
		cout << "Подключенные девайсы:\n";

		for (UsbDeviceDesc it : deviceArray)
			cout << deviceId++ << ". " << it.friendlyName << (it.removable ? "\t(Извлекаемый)" : "\t(Не извлекаемый)") << endl;
		cout << ">";
		choice = _getch() - '0';
		...
		{
			cout << "Девайс " << static_cast<int>(choice) << " НЕ является извлекаемым!\n";
			system("pause");
		}
	}
